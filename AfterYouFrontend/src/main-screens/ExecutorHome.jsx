    ...
    <div className="executor-home">
      ...
      <main className="content">
        <div className="welcome-section">
          ...
          <p className="description">Manage and execute assigned wills securely and efficiently.</p>
        </div>

        <hr className="solid-divider" /> {/* Solid line divider */}

        <h2 className="accounts-heading">Accounts</h2>
        <div className="account-list">
          {linkedAccounts.map((account, index) => (
            <div className="account-card" key={index}>
              <FontAwesomeIcon className={`account-icon ${account.name.toLowerCase()}`} icon={account.icon} size="2x" />
              <div className="account-details">
                <h3>{account.name}</h3>
                <p>Profile | {account.profile}</p>
              </div>
              {/* Just Download Icon, No Click Action */}
              <FontAwesomeIcon className="download-icon" icon={faDownload} size="2x" />
            </div>
          ))}
        </div>
      </main>
